...
test.describe('Authentication — unauthenticated', () => {
  // These tests need a clean session (no storageState) to see the auth page
...
test.describe('Authentication — authenticated', () => {
  // These tests use the default storageState with the session token
  ...
  // SKIP: sign out revokes the Supabase refresh token server-side,
  // which invalidates the shared session for all subsequent tests.
  // This test should only run in isolation or as the very last test.
  test.skip('sign out works', async ({ page }) => {
    await login(page);
    await logout(page);
    const url = page.url();
