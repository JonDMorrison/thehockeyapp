...
const navItems: NavItem[] = [
  { icon: Home, labelKey: "nav.home", path: "/today" },
  { icon: Users, labelKey: "nav.teams", path: "/teams" },
  { icon: User, labelKey: "nav.players", path: "/players" },
          ...
          <div className="flex items-center justify-around h-16 max-w-lg mx-auto relative">
            {navItems.map((item) => {
              const isActive = location.pathname === item.path ||
                (item.path !== "/today" && location.pathname.startsWith(item.path)) ||
                (item.path === "/today" && (location.pathname === "/today" || location.pathname.startsWith("/today/") || location.pathname.startsWith("/solo/")));
