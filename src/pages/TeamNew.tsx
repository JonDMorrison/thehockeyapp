...
const teamSchema = z.object({
  name: z.string().trim().min(1, "Enter a team name").max(100, "Team name must be 100 characters or fewer"),
  season_label: z.string().trim().max(50, "Season label must be 50 characters or fewer").optional(),
  palette_id: z.string(),
});
...
const TeamNew: React.FC = () => {
  ...
  const validate = () => {
    ...
    setErrors(newErrors);
    focusFirstInvalidField(newErrors, { name: "name", season_label: "season_label" });
    return false;
  };
          ...
          <AppCard>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="name">{t("teams.new.teamNameLabel")}<RequiredMark /></Label>
                <Input
                  id="name"
                  value={formData.name}
                ...
                />
                {errors.name && (
                  <p id="team-name-error" role="alert" className="text-xs text-destructive">{errors.name}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  maxLength={50}
                  className={errors.season_label ? "border-destructive" : ""}
                  aria-invalid={Boolean(errors.season_label)}
                  aria-describedby={errors.season_label ? "season-label-error" : undefined}
                />
                {errors.season_label && <p id="season-label-error" role="alert" className="text-xs text-destructive">{errors.season_label}</p>}
              </div>
            </div>
          </AppCard>
