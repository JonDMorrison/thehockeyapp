...
DECLARE
  ...
  v_onboarding record;
  v_game_event record;
  v_practice_card_found boolean := false;
BEGIN
  v_user_id := auth.uid();
  ...
  IF NOT v_practice_card_found THEN
    SELECT id, title, tier, mode, published_at INTO v_practice_card
    FROM public.practice_cards
    WHERE team_id = p_team_id AND date = v_today AND (mode = 'normal' OR mode IS NULL)
    LIMIT 1;
  ...
  RETURN jsonb_build_object(
    ...
    'today', jsonb_build_object(
      ...
      'game_day', jsonb_build_object(
        ...
        'event_time', v_game_event.start_time,
        'opponent', v_game_event.title
      ),
      'practice_card', jsonb_build_object(
        'exists', v_practice_card_found,
        'published', v_practice_card_found AND v_practice_card.published_at IS NOT NULL,
        'card_id', CASE WHEN v_practice_card_found THEN v_practice_card.id ELSE NULL END,
        'title', CASE WHEN v_practice_card_found THEN v_practice_card.title ELSE NULL END
      )
    ),
    ...
    'onboarding', jsonb_build_object(
      ...
      'checklist', jsonb_build_array(
        jsonb_build_object('id', 'set_preferences', 'label', 'Set training preferences', 'done', v_preferences_exists),
        jsonb_build_object('id', 'connect_schedule', 'label', 'Connect schedule', 'done', v_schedule_exists),
        jsonb_build_object('id', 'add_players', 'label', 'Add players', 'done', v_roster_count > 0),
        jsonb_build_object('id', 'invite_parents', 'label', 'Invite parents', 'done', v_invite_exists)
      )
    )
  );
END;
